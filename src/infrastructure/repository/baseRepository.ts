...
import { IBaseRepository } from "../../domain/repositoryInterface/baseRepository.interface";
...
export class BaseRepository<TDoc extends Document, TEntity>
    ...
    async updateById(id: string, data: Partial<TEntity>): Promise<TEntity | null> {
        const doc = await this.model.findByIdAndUpdate(id, this.toModel ? this.toModel(data) : (data as any), { new: true })
            .exec()
        if (!doc) return null;
        return this.toDomain ? this.toDomain(doc) : (doc as any)
